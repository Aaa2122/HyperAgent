...
def test_model_explicit_notional_is_preserved_by_sizing() -> None:
    ...
    decision = AssetDecision(
        symbol="BTC", action="OPEN", direction="LONG", size_frac=1,
        leverage=4, notional_usd=12_345, confidence=.8,
    ...
    sized = size_open_order(
        ...
        LLMLayerConfig(max_asset_notional_usd={"BTC": 1e9, "ETH": 1e9, "SOL": 1e9}, max_net_exposure_frac=1e9),
    )
...
def test_consequence_report_is_descriptive_and_counterfactual() -> None:
    ...
    sheet = FeatureSheet(
        as_of=fixtures.NOW,
        assets=[fixtures.make_features("BTC", 50_000), fixtures.make_features("ETH", 3_000), fixtures.make_features("SOL", 155)],
        corr_30d_btc_eth=.87,
        corr_30d_btc_sol=.74,
    )
    ...
    assert [item.size_multiplier for item in btc.scenarios] == [.5, 1, 1.5]
...
def test_operational_mode_ignores_strategic_confidence_but_requires_notional() -> None:
    plan = PlaybookLLMOutput(**fixtures.valid_playbook_dict()).plan_for("BTC")
    snapshot = AssetSnapshot(symbol="BTC", mark_px=49_800, atr_4h=800, spread_bps=1, data_age_seconds=2, max_leverage=20)
    portfolio = PortfolioContext(equity_usd=10_000, positions=[])
    cfg = LLMLayerConfig(operational_only=True)
    no_notional = AssetDecision(symbol="BTC", action="OPEN", direction="LONG", size_frac=.2, confidence=0, rationale="Low conviction is still a discretionary strategic choice here.")
    from llm_checks import check_decision
    playbook = fixtures.make_record(PlaybookLLMOutput(**fixtures.valid_playbook_dict()))
    assert "NOTIONAL_NOT_EXPLICIT" in {item.code for item in check_decision(no_notional, plan, playbook, snapshot, portfolio, cfg, fixtures.NOW)}
    explicit = no_notional.model_copy(update={"notional_usd": 1_000})
    assert "LOW_OPEN_CONFIDENCE" not in {item.code for item in check_decision(explicit, plan, playbook, snapshot, portfolio, cfg, fixtures.NOW)}
