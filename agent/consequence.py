    ...
    *,
    ...
    for decision in trader.decisions:
        ...
        if decision.action != "OPEN" or decision.direction is None:
            reports.append(DecisionConsequences(
        ...
        liquidation = asset.mark_px * (1 - 1 / leverage) if long else asset.mark_px * (1 + 1 / leverage)
        liq_to_stop_atr = abs(liquidation - stop) / max(asset.atr_4h, 1e-12)
        funding_cost = notional * (asset.funding_1h_pct / 100) * decision.horizon_hours * (1 if long else -1)
        ...
        for multiplier in (0.5, 1.0, 1.5):
            ...
            scenarios.append(ConsequenceScenario(
                size_multiplier=multiplier,
                notional_usd=scenario_notional,
                stop_loss_usd=scenario_qty * stop_distance,
                stop_loss_equity_pct=(scenario_qty * stop_distance / max(equity_usd, 1e-12)) * 100,
