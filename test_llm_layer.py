...
def valid_playbook_dict() -> dict:
    return dict(
        ...
        plans=base_plans(),
        changes_vs_previous="First playbook of the run; no previous version "
                            "to diff against.",
        ttl_hours=8,
    )
...
def valid_trader_dict() -> dict:
    return dict(
        decisions=[
            dict(symbol="BTC", action="OPEN", direction="LONG", size_frac=0.75,
                 confidence=0.7,
                 rationale="Mark 49800 inside 49200-50100 zone, stop 1.75 ATR "
                           "below, funding benign."),
            dict(symbol="ETH", action="HOLD", confidence=0.4,
                 rationale="FLAT plan and no position; nothing to do."),
            dict(symbol="SOL", action="HOLD", confidence=0.5,
                 rationale="Price below entry zone 152-158; no valid setup "
                           "this cycle."),
        ],
    )
...
def make_record(pb: PlaybookLLMOutput, expired: bool = False) -> PlaybookRecord:
    return PlaybookRecord(
        ...
        model_id="frontier-x", prompt_version="strategist-v1",
        payload=pb,
    )
...
def test_structural_trader() -> None:
    ...
    rejected("OPEN sans direction",
             lambda: TraderOutput(**patch(0, direction=None)))
    rejected("OPEN avec size_frac = 0",
             lambda: TraderOutput(**patch(0, size_frac=0.0)))
    rejected("CLOSE avec size_frac > 0",
             lambda: TraderOutput(**patch(1, action="CLOSE", size_frac=0.5)))
    rejected("REDUCE avec direction",
             lambda: TraderOutput(**patch(1, action="REDUCE", size_frac=0.5,
                                          direction="LONG")))
    rejected("HOLD avec size_frac > 0",
             lambda: TraderOutput(**patch(2, size_frac=0.3)))
    rejected("review demandé sans raison",
             lambda: TraderOutput(**{**valid_trader_dict(),
                                     "request_strategist_review": True}))
    rejected("raison sans flag de review",
             lambda: TraderOutput(**{**valid_trader_dict(),
                                     "review_reason": "Funding flipped hard on SOL."}))
    rejected("actif manquant (BTC dupliqué)",
             lambda: TraderOutput(**patch(1, symbol="BTC")))
...
def test_contextual_plan() -> None:
    ...
    cfg = LLMLayerConfig()
    pb = PlaybookLLMOutput(**valid_playbook_dict())
    snap = AssetSnapshot(symbol="BTC", mark_px=49800.0, atr_4h=800.0,
                         spread_bps=1.2, data_age_seconds=5.0)

    ok("plan BTC cohérent avec le marché → aucune violation",
       check_plan_against_market(pb.plan_for("BTC"), snap, cfg) == [])

    wrong_side = AssetPlan(
        symbol="BTC", bias="LONG", conviction=0.6,
        thesis="Long idea with a stop incorrectly placed above the current "
               "market price for this test.",
        invalidation_px=50500.0, risk_alloc=0.3,
    )
    ...
    too_wide = AssetPlan(
        ...
        invalidation_px=44000.0, risk_alloc=0.3,
    )
    ...
    behind = AssetPlan(
        ...
        thesis="Long idea whose first target is already below the current "
               "market price for this test.",
        invalidation_px=48400.0, targets=[49000.0, 52000.0], risk_alloc=0.3,
    )
...
def test_contextual_decision() -> None:
    ...
    pf_with_pos = PortfolioContext(
        equity_usd=10_000.0,
        positions=[PositionState(symbol="BTC", side="LONG", notional_usd=1_500.0,
                                 entry_px=49_500.0, invalidation_px=48_400.0)],
    )

    d_open = AssetDecision(symbol="BTC", action="OPEN", direction="LONG",
                           size_frac=0.75, confidence=0.7,
                           rationale="Inside entry zone, stop 1.75 ATR below, "
                                     "funding benign.")
    ok("OPEN conforme → aucune violation",
       check_decision(d_open, plan_btc, record, snap, pf_empty, cfg, NOW) == [])

    codes = [x.code for x in check_decision(
    ...
    d_short = AssetDecision(symbol="BTC", action="OPEN", direction="SHORT",
                            size_frac=0.5, confidence=0.8,
                            rationale="Deliberate mismatch against the playbook "
                                      "bias for this test.")
    codes = [x.code for x in check_decision(
        d_short, plan_btc, record, snap, pf_empty, cfg, NOW)]
    ok("OPEN contre le bias bloqué", "BIAS_MISMATCH" in codes)

    d_flat = AssetDecision(symbol="ETH", action="OPEN", direction="LONG",
                           size_frac=0.5, confidence=0.8,
                           rationale="Attempt to open on a FLAT plan for this "
                                     "test case.")
    snap_eth = AssetSnapshot(symbol="ETH", mark_px=3_000.0, atr_4h=60.0,
                             spread_bps=1.5, data_age_seconds=5.0)
    codes = [x.code for x in check_decision(
        d_flat, pb.plan_for("ETH"), record, snap_eth, pf_empty, cfg, NOW)]
    ok("OPEN sur plan FLAT bloqué", "BIAS_FLAT" in codes)

    codes = [x.code for x in check_decision(
        d_open, plan_btc, record, snap, pf_with_pos, cfg, NOW)]
    ok("OPEN avec position déjà ouverte bloqué", "ALREADY_IN_POSITION" in codes)

    snap_stale = AssetSnapshot(symbol="BTC", mark_px=49800.0, atr_4h=800.0,
                               spread_bps=1.2, data_age_seconds=120.0)
    codes = [x.code for x in check_decision(
        d_open, plan_btc, record, snap_stale, pf_empty, cfg, NOW)]
    ok("données périmées bloquent OPEN (I4)", "DATA_STALE" in codes)

    d_close = AssetDecision(symbol="BTC", action="CLOSE", confidence=0.9,
                            rationale="Exiting the full position after target "
                                      "touch in this test.")
    ok("CLOSE passe même sur données périmées (asymétrie voulue)",
       check_decision(d_close, plan_btc, record, snap_stale, pf_with_pos,
                      cfg, NOW) == [])
    codes = [x.code for x in check_decision(
        d_close, plan_btc, record, snap, pf_empty, cfg, NOW)]
    ok("CLOSE sans position bloqué", "NO_POSITION" in codes)

    snap_far = AssetSnapshot(symbol="BTC", mark_px=51_000.0, atr_4h=800.0,
                             spread_bps=1.2, data_age_seconds=5.0)
    codes = [x.code for x in check_decision(
        d_open, plan_btc, record, snap_far, pf_empty, cfg, NOW)]
    ok("prix hors zone d'entrée bloqué", "OUT_OF_ENTRY_ZONE" in codes)

    d_shy = AssetDecision(symbol="BTC", action="OPEN", direction="LONG",
                          size_frac=0.5, confidence=0.4,
                          rationale="Low-confidence open attempt for this "
                                    "test case.")
    codes = [x.code for x in check_decision(
        d_shy, plan_btc, record, snap, pf_empty, cfg, NOW)]
    ...
    codes = [x.code for x in check_decision(
        d_open, plan_btc, record, snap, pf_busy, cfg, NOW)]
    ok("limite d'OPEN journalière bloquée", "OVERTRADE_LIMIT" in codes)

    pf_cool = PortfolioContext(equity_usd=10_000.0,
                               minutes_since_stop_out={"BTC": 30.0})
    codes = [x.code for x in check_decision(
        d_open, plan_btc, record, snap, pf_cool, cfg, NOW)]
...
def test_sizing() -> None:
    ...
    plan = AssetPlan(
        ...
        thesis="Reference long plan used for the numeric sizing example of "
               "the annex document.",
        invalidation_px=49_000.0, risk_alloc=0.5,
    )
    snap = AssetSnapshot(symbol="BTC", mark_px=50_000.0, atr_4h=800.0,
                         spread_bps=1.0, data_age_seconds=3.0)
    d = AssetDecision(symbol="BTC", action="OPEN", direction="LONG",
                      size_frac=1.0, confidence=0.8,
                      rationale="Full deployment of the planned risk for the "
                                "sizing example.")

    pf = PortfolioContext(equity_usd=10_000.0)
    so = size_open_order(d, plan, snap, pf, cfg)
    # risque = 10000 × 2% × 0.5 × 1.0 = 100 $ ; stop à 1000 $ → qty 0.1 BTC
    # → notionnel brut 5000 $ → cap actif BTC 2000 $
    ok("risque calculé = 100 $", abs(so.risk_usd - 100.0) < 1e-6)
    ok("notionnel cappé au cap actif (2000 $)",
       abs(so.notional_usd - 2_000.0) < 1e-6)
    ok("raison ASSET_NOTIONAL_CAP présente",
       "ASSET_NOTIONAL_CAP" in so.cap_reasons and so.was_capped)

    pf_loaded = PortfolioContext(
        equity_usd=10_000.0,
        positions=[PositionState(symbol="ETH", side="LONG", notional_usd=5_500.0,
                                 entry_px=3_000.0, invalidation_px=2_850.0)],
    )
    so2 = size_open_order(d, plan, snap, pf_loaded, cfg)
    # cap net = 60% × 10000 = 6000 $ ; net actuel +5500 → headroom 500 $
    ok("cap d'exposition nette applique le headroom (500 $)",
       abs(so2.notional_usd - 500.0) < 1e-6)
    ok("raison NET_EXPOSURE_CAP présente", "NET_EXPOSURE_CAP" in so2.cap_reasons)

    d_tiny = AssetDecision(symbol="BTC", action="OPEN", direction="LONG",
                           size_frac=0.004, confidence=0.8,
                           rationale="Tiny deployment to trigger the minimum "
                                     "notional floor.")
    so3 = size_open_order(d_tiny, plan, snap, pf, cfg)
    # risque 0.4 $ → notionnel 20 $ < plancher 25 $ → 0 (REJECT)
    ok("sous le plancher → notionnel 0 (REJECT)",
...
def make_features(symbol: str, mark: float) -> dict:
    return dict(
        symbol=symbol, mark_px=mark,
        ret_1h_pct=0.2, ret_4h_pct=0.9, ret_1d_pct=2.1, ret_7d_pct=5.4,
        atr_4h=mark * 0.016, adx_4h=27.0, donchian_pos_4h=0.82,
        dist_ema20_4h_atr=1.3, dist_ema200_1d_atr=2.9,
        rv_24h_ann_pct=41.0, rv_7d_ann_pct=38.0,
        funding_1h_pct=0.008, funding_24h_avg_pct=0.006,
        oi_usd=1.9e9, oi_change_24h_pct=6.0,
        liq_longs_24h_usd=1.1e7, liq_shorts_24h_usd=2.3e7,
        swing_high_4h=mark * 1.03, swing_low_4h=mark * 0.96, vwap_1d=mark * 0.995,
        spread_bps=1.1, data_age_seconds=4.0,
        advisors=[dict(strategy_id="naive_momentum", direction="LONG",
                       conviction=0.65, features={"donchian_break_atr": 0.8})],
    )
...
def test_feature_sheet() -> None:
    ...
    fs = FeatureSheet(
        as_of=NOW,
        assets=[make_features("BTC", 50_000.0),
                make_features("ETH", 3_000.0),
                make_features("SOL", 155.0)],
        corr_30d_btc_eth=0.87, corr_30d_btc_sol=0.74,
    )
    ok("feature sheet valide parse (3 actifs, advisors inclus)",
       len(fs.assets) == 3)
    rejected("feature sheet incomplet rejeté",
             lambda: FeatureSheet(
                 as_of=NOW,
                 assets=[make_features("BTC", 50_000.0),
