...
export function TradeHistoryPanel({
  ...
  return (
    <div>
      ...
      <div className="mt-7 grid grid-cols-2 gap-x-7 gap-y-5 border-y border-white/[.07] py-6 md:grid-cols-4 xl:grid-cols-7">
        {stats.map(([label, value], index) => (
          <div key={label} className={index ? "md:border-l md:border-white/[.06] md:pl-5" : ""}>
            <p className="eyebrow">{label}</p>
            <p className="mt-2 font-mono text-base text-white/85">{value}</p>
          </div>
        ))}
      </div>

      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <label className="flex min-h-11 flex-1 items-center gap-2 border-b border-white/[.09] text-white/45 sm:max-w-xs">
          <Search className="h-3.5 w-3.5" />
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Rechercher un actif"
            className="h-11 w-full bg-transparent text-xs text-white outline-none placeholder:text-white/25"
          />
        </label>
        <div className="flex flex-wrap gap-2">
          {(["ALL", "PROFIT", "LOSS", "BREAK_EVEN"] as const).map((value) => (
            <button
              ...
              className={`min-h-10 rounded-full px-3 text-[10px] transition ${
                outcome === value
                  ? "bg-white text-black"
          ...
          <label className="relative flex min-h-10 items-center rounded-full bg-white/[.04] px-3 text-[10px] text-white/45">
            <select
              value={sort}
      ...
      <div className="mt-5 border-t border-white/[.07]">
        {trades.length ? (
          trades.map((trade) => <TradeRow key={trade.trade_id} trade={trade} />)
        ) : (
          <div className="grid min-h-64 place-items-center text-center">
            <div>
              <span className="mx-auto grid h-11 w-11 place-items-center rounded-full bg-white/[.04] text-white/30">
    ...
    <details className="group">
      <summary className="grid min-h-16 cursor-pointer list-none grid-cols-[1fr_auto] items-center gap-4 md:grid-cols-[.7fr_.8fr_.8fr_.75fr_.75fr_auto]">
        <div className="flex items-center gap-3">
          <span className={`grid h-8 w-8 place-items-center rounded-full ${positive ? "bg-[#30d158]/10 text-[#30d158]" : neutral ? "bg-white/[.05] text-white/35" : "bg-[#ff453a]/10 text-[#ff6961]"}`}>
            {positive ? <ArrowUpRight className="h-3.5 w-3.5" /> : <ArrowDownRight className="h-3.5 w-3.5" />}
          </span>
          <div>
            <p className="text-sm font-medium">{trade.symbol}</p>
            <p className="text-[10px] text-white/30">{trade.side} · {trade.leverage}×</p>
          </div>
        </div>
        <div className="hidden md:block">
          <p className="font-mono text-xs">{price.format(trade.avg_entry_px)}</p>
          <p className="text-[9px] text-white/25">entrée moyenne</p>
        </div>
        <div className="hidden md:block">
          <p className="font-mono text-xs">{price.format(trade.avg_exit_px)}</p>
          <p className="text-[9px] text-white/25">sortie moyenne</p>
        </div>
        <div className="hidden md:block">
          <p className="text-xs">{date.format(new Date(trade.closed_at))}</p>
          <p className="text-[9px] text-white/25">clôture</p>
        </div>
        <div className="hidden md:block">
          <p className={`font-mono text-xs ${trade.margin_return_pct >= 0 ? "text-[#30d158]" : "text-[#ff6961]"}`}>
            {trade.margin_return_pct >= 0 ? "+" : ""}{trade.margin_return_pct.toFixed(2)}%
          </p>
          <p className="text-[9px] text-white/25">sur marge</p>
        </div>
        <div className="text-right">
          <p className={`font-mono text-sm ${positive ? "text-[#30d158]" : neutral ? "text-white/50" : "text-[#ff6961]"}`}>
            {positive ? "+" : ""}{usd.format(trade.net_pnl_usd)}
          </p>
          <p className="text-[9px] text-white/25">net</p>
        </div>
      </summary>
      <div className="grid gap-5 pb-5 pt-2 text-xs text-white/45 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {(trade.thesis || trade.rationale) && (
          <div className="sm:col-span-2 lg:col-span-4">
            ...
            <p className="mt-2 max-w-4xl leading-relaxed text-white/55">
              {trade.thesis || trade.rationale}
            </p>
          </div>
        )}
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="eyebrow">{label}</p>
      <p className="mt-1.5 font-mono text-white/65">{value}</p>
    </div>
  );
}
